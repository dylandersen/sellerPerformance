...
import getSellerPerformance from "@salesforce/apex/SellerPerformanceCopyController.getSellerPerformance";
import { ShowToastEvent } from "lightning/platformShowToastEvent";
import AGENTFORCE_ICON from "@salesforce/resourceUrl/AgentforceRGBIcon";

export default class SellerPerformanceCopy extends LightningElement {
  ...
  get totalPipelineValueFormatted() {
    const value = this.totalPipelineValue;
    if (value >= 1000000) {
  ...
  get dealVelocity() {
    ...
    return velocity != null ? Number(velocity) : 0;
  }
  ...
  get healthyDealsCount() {
    ...
    const totalOpen = (this.metrics.opportunitiesCount || 0) - 
  ...
  get velocityBadgeClass() {
    ...
    if (status === "Fast" || status === "Good") return `${baseClass} velocity-fast`;
